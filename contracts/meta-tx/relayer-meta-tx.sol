...
contract RelayerMetaTransaction {
    ...
    address public owner;  // Menyimpan pemilik kontrak
    mapping(address => uint256) public nonces;  // Menyimpan nonce untuk setiap pengguna
    mapping(address => bool) public relayerWhitelist;  // Menyimpan daftar relayer yang diperbolehkan
    ...
    constructor() {
        uint256 chainId;
        assembly {
            chainId := chainid()  // Mendapatkan ID chain saat ini
        }
        ...
        owner = msg.sender;  // Menetapkan pemilik kontrak ke alamat pembuat kontrak
    }
    ...
    function executeMetaTransaction(
        ...
        // Membuat digest untuk verifikasi tanda tangan
        ...
        if (fee > 0) {
            if (msg.value >= fee) {
                // Jika relayer membayar dengan ETH
                (bool sent, ) = msg.sender.call{value: fee}("");
                require(sent, "Fee transfer failed");
            } else {
                // Jika relayer membayar dengan token ERC-20
        ...
        nonces[user]++;  // Increment nonce untuk pengguna
